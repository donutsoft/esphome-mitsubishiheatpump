...
An energy saver mode has also been added. When enabled, ESP32 will gradually increase the set temperature by no more than 1.5 degrees celsius above room temperature. This has the tradeoff that warming rooms is much slower, but the overall energy consumed should be less.
...
## Energy saver mode
Add the following switch to make energy saving mode controllabe.
...
switch:
    ...
    lambda: |-
      if (id(basementhp).get_energy_saving_mode()) {
        return true;
      } else {
        return false;
      }
    turn_on_action:
      then:
        - lambda: 'id(basementhp).set_energy_saving_mode(true);'
    turn_off_action: 
      then:
        - lambda: 'id(basementhp).set_energy_saving_mode(false);'
```
